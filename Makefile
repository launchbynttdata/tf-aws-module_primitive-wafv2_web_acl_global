...
define create_example_providers
	...
	$(eval INCLUDE_AWS_GLOBAL:=$(findstring aws.global,$(shell grep -se "\\s*provider\\s*=" *.tf || true)))
...
.PHONY: tfmodule/lint
# Skip terraform validate on `.` when this module uses configuration_aliases
# (aws.global). Validating the root as a standalone config needs provider.tf,
# which then blocks examples from passing providers = { aws.global = aws.global }.
tfmodule/lint: tfmodule/init
	@$(call check_terraform_fmt)
	@$(foreach module,$(ALL_TF_MODULES),$(call tflint_terraform_module,$(module)))
	@$(foreach module,$(filter-out .,$(ALL_TF_MODULES)),$(call validate_terraform_module,$(module)))
